...
fn main() {
    ...
    let ready = matches
        .value_of("ready")
        .map(|s| Regex::new(s).expect("bad regex for ready"));
    ...
    loop {
        let length = match input.read_line(&mut buf) {
            Err(error) => {
        ...
        output.flush().expect("failed to flush stream");
        buf.clear();
    }
}
